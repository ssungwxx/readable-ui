...
| `destructive-action` | `"two-step-preview"` (default) | destructive tool 호출은 **preview tool(1단계) → confirm 페이지 Form 제출(2단계)** 2단계 전이로 작동한다. envelope `tools[]` 에 `<verb><Resource>Preview` 와 `<verb><Resource>` 가 함께 등장하면 한 쌍의 idiom — 독립 호출이 아님. preview 응답 페이지는 `intent: "destructive-confirm"` 로 마킹 (ADR 0020 §2·§5). |
