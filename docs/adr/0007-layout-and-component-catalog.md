...
- Status: Accepted (§2 revised by [ADR 0011](./0011-sidebar-and-topbar-page-layouts.md); §3 Table classification revised by [ADR 0015](./0015-table-as-container-directive.md))
- Date: 2026-04-17
...
- `Card` (독립 영역)
- `Form` (액션 묶음)
- `Table` (ADR 0015: `:::table{tool=... page= of= sort= filter-*=...}` + 내부 GFM pipe table)
...
- 카탈로그 내부에서 컴포넌트의 **직렬화 형태(Block ↔ Container)를 바꾸는 재정의**는 일반적으로 허용하지 않는다. ADR 0015에서 Table을 Block→Container로 전환한 것은 admin 자동화 요구에 맞춰 본 ADR이 **단일 예외로 승인한 재정의**이며, 다른 컴포넌트에는 전이하지 않는다.
