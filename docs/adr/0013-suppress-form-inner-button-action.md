...
## Follow-up — envelope convention 키 자동 주입 (2026-04-18)

ADR 0015 LLM 재검증(`docs/research/llm-test-0015-table-container.md` T9)에서 이 규약이 여전히 medium confidence로 관찰됐다. 구조 추론으로도 정답에 도달하나, ADR 0012 `duplicate-button-link` 선례처럼 **envelope에 규범을 외재화**하면 self-explanatory 수준이 high로 고정된다.

### 추가 결정

`renderPage`가 envelope에 `extensions.conventions["form-inner-button-action"]`가 없으면 기본값 `"inherit"`을 자동 주입한다. ADR 0012 §A와 동일 메커니즘.
...
의미: "Form 내부의 Button이 `action=` 속성을 갖지 않으면 감싸는 `:::form{action=X}` 의 action을 상속해 같은 tool을 1회 호출한다. 명시적으로 다른 `action=` 을 선언하는 Button은 해당 action을 따로 호출한다."

- `page-envelope.md` §extensions.conventions 표에 키 추가.
- `packages/core/src/index.ts` 의 `withDefaultConventions` 가 두 키를 모두 주입.
- 런타임 동작·Button 직렬화 경로는 변경 없음.
